...
export default class Simulator {
  ...
  run(frontcallback: any) {
    if(this.agentListChanged){
      this.agentsinuse=this.agents.filter((value)=>{
        return value.enable
      })
      this.agentListChanged=false
    }
    this.kdTree.buildAgentTree();
    let c;
    for (var i = 0; i < this.agentsinuse.length; i++) {
      c=this.agentsinuse[i];
      frontcallback(c.id);
      c.computeNeighbors();
      c.computeNewVelocity();
    }
